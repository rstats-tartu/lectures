...
```{r, eval=FALSE, fig.width=5, fig.height=5, message=FALSE}
library(ggExtra)
...
tibble(CI_percentage = c(50, 75, 90, 95, 97, 99, 99.9), 
       nr_of_SEMs = c(0.675, 1.15, 1.645, 1.96, 2.17, 2.575, 3.291)) %>% knitr::kable()
```
...
Eesmärk on näidata hulkade ühisosa e kattuvust. Tavaliselt tehakse seda Venni diagraamiga. Meie kasutame paketti ComplexHeatmap, mis võtab andmed sisse listina või andmeraamina. Listist saab konverteerida andmeraamiks list_to_matrix() abil.

```{r}
lt = list(set1 = c("a", "b", "c"),
          set2 = c("b", "c", "d", "e"))
lt1 <- list_to_matrix(lt)
```

make_comb_mat() teeb kombinatsioonimaatriksi ja arvutab kombinatsioonide arvud. 
...
Kombinatsioonide arvu saab piirata filtreerides kombinatsioonide suuruse järgi. `m1 = make_comb_mat(lt, min_set_size = 6)` min_set_size 6 elementi, või
`m2 = make_comb_mat(lt, top_n_sets = 2)` top_n_sets annab 2 hulka, millel on kõige rohkem elemente.
...
pred_matrix <- add_predictions(pred_matrix, m2)
ggplot(pred_matrix, aes(x = Petal.Length)) + 
  geom_point(data= iris, aes(y = Sepal.Length)) +
  geom_line(aes(y = pred))
```
