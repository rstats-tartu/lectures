...
## 2.2.1 Mudeli väike maailm
  ...
  Lisaks, tõenäosusteooria ütleb, et Bayesi teoreemi kasutades töötame me mudelis leiduva informatsiooniga parimal võimalikul viisil. Aga kogu see rõõm jääb ikkagi meie mudeli piiridesse.
...
Aga mis juhtub, kui me lisame mudelile konstandi: *y = a + x*?
```{r fig.height=3, fig.width=3}
x <- 0:200 
a <- 30
y <- a + x 
...
Mis juhtub, kui me mitte ei liida vaid korrutame *x*-i konstandiga: *y =  bx*?
...
**Ja lõpuks kaks tavalist lineaarse mudeli laiendust.** Esiteks, mudel, kus meil on mitu prediktorit x1, x2, ... xn, mis on additiivse mõjuga. Additiivne tähendab, et me liidame nende mõjud, mis omakorda tähendab, et me usume, et x1...xn mõjud y-i väärtusele on üksteisest sõltumatud. Mudel on siis kujul *y = a + b1x1 + b2x2 + ... + bnxn*

me ehitame lihtsa variandi sellest mudelist *y = x1 + x2* (a = b1 = b2 = 0) ja plotime selle mudeli y sõltuvusena x1-st juhul, kus x2 on fikeeritud oma keskmisele väärtusele 
...
plot(y~x1, 
     type="l", 
     ylim=c(0, 200), col="red")
lines(y1~x1)
```

Nagu näha, tulemuseks on ikkagi sirge. Veelgi enam, tulemuseks on paralleelne sirge mudeliga y=x1, mille intercept = mean(x2). Muidugi võime me plottida selle mudeli suvalise x2 väärtuse korral või lasta muutuma hoopis x2 ja fikseerida x1 väärtuse. 

Me võime ka joonistada 3D pildi olukorrast, kus nii x1 kui x2 omandavad rea väärtusi:
...
scatterplot3d(df$x1, df$x2, df$y, highlight.3d=TRUE, col.axis="blue",
      col.grid="lightblue")
```

Selles mudelis erineb sirge y = x1 + x2 tõus sirge y= x1 tõusust, aga tegemist on ikkagi sirgega 3D ruumis.


Nüüd lisame eelmisele mudelile interaktsiooni kahe muutuja (x1 ja x2) vahel.

*y = y = x1 + x2 + b3x1x2*

Interaktsiooni modelleerime korrutades läbi x1 ja x2 ja andes sellele korrutisele kaalu b3 (antud juhul 0.1). Jällegi plotime kõigepealt olukorra, kus x1 omab rida väärtusi, aga x2 on fikseeritud oma keskväärtusele.
```{r fig.height=3, fig.width=3}
x1 <-  1:100 
x2 <-  1:100
b3 <- 0.1
...
Nagu näha, interaktsioon viib selleni, et interaktsioonimudeli tõus erineb ilma interaktsioonita mudeli tõusust. Kui aga interaktsioonimudel plottida välja 3D-s üle paljude x1 ja x2 väärtuste, saame me regressioonikurvi (mitte sirge), kus b3 annab kurvatuuri.
...
y <-   x1 + x2 + b3*x1*x2
y1 <-  x1 + x2 
df <- data.frame(y=y, y1=y1, x1=x1, x2=x2)
scatterplot3d(df$x1, df$x2, df$y, highlight.3d=TRUE, col.axis="blue",
      col.grid="lightblue")
```
