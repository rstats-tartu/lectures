...
**Järgmiseks näitame kaht tavalist lineaarse mudeli laiendust.** 

Esiteks, mudel, kus meil on mitu prediktorit x1, x2, ... xn, mis on additiivse mõjuga. Additiivne tähendab, et me liidame nende mõjud, mis omakorda tähendab, et me usume, et x1...xn mõjud y-i väärtusele on üksteisest sõltumatud. Mudel on siis kujul 
...
scatterplot3d(df$x1, df$x2, df$y, 
              col.axis="blue", 
              col.grid="lightblue", 
              type="l", lwd=2, 
      zlim=c(0, 150), xlim=c(0, 150), ylim=c(0, 150))
```

Selles mudelis erineb sirge *y = x1 + x2* tõus sirge *y= x1* tõusust, aga tegemist on ikkagi sirgega -- mis sest, et 3D ruumis.
...
Nagu näha, korrutamistehe viib selleni, et interaktsioonimudeli tõus erineb ilma interaktsioonita mudeli tõusust. Kui aga interaktsioonimudel plottida välja 3D-s üle paljude x1 ja x2 väärtuste, saame me regressioonikurvi (mitte sirge), kus b3 annab kurvatuuri.
...
y <-   x1 + x2 + b3*x1*x2
df <- data.frame(y=y,  x1=x1, x2=x2)
scatterplot3d(df$x1, df$x2, df$y, col.axis="blue",
      col.grid="lightblue", type="l", lwd=2,
      zlim=c(0, 150), xlim=c(0, 150), ylim=c(0, 150))
```

Ja lõpuks painutame oma sirge mudelit polünoomide abil.

*y = a + bx + b1x^2 + b2x^3 + ... + bnx^n*

See joon on ikka veel täielikult fikseeritud, aga ta pole enam sirge (ehkki tehniliselt on meil ikka lineaarne seos x ja y vahel). Me kasutame konstanti -0.012, et saada meile meeldiva kujuga joont, aga isegi kui see joon sobib hästi andmetega, on vähetõenäoline, et selle mudeli matemaatiline formalism aitaks meil oma teaduslikku teooriat paremini mõista. Polünoomsed mudelid on äärmiselt paindlikud, kuid teaduslikus mõttes ebahuvitavad senikaua, kui nende struktuur ei kajasta tuntud füüsikalisi protsesse. 
```{r fig.height=3, fig.width=3}
x <- 0:100
y <- x - 0.012*x**2  
plot(y~x, 
