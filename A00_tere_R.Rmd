...
## 1.2 R on kalkulaator

Liidame `2 + 2`. 

     Selleks, et R aru saaks, et tegu on temale mõeldud käsuga, tuleb see kirjutada
     Insert --> R abil (vt kohe ülesse rohelisele nupule "Insert") ja kood evalueeritakse 
     siis, kui vajutad tekinud hallis koodialas rohelisele nupule või alternatiivselt, 
     kui soovid jooksutada ainult osa koodi või üht koodirida, siis pane kursor soovitud 
     reale või võta osa koodi blokki ja vajuta klaviatuuril cmd + enter.
...
## 1.3 R on andmeanalüüsi keel, mille verbid on funktsioonid
...
Seega tasub alati kontrollida, kas R ikka tegi seda, mida sina talle enda arust ette kirjutasid.
    ...
    R-i põhiandmestruktuur on vektor: järjestatud rida elemente (numbreid, tähemärke, jm). 
    Vektor elementidega 2, 3.4 ja 4 kirjutatakse c(2, 3.4, 4). 
...
sum(x)
sum(x, y)
prod(x,y) #x, y ja z produkt ehk x * y 
mean(x)
median(x)
quantile(x)
...
## 1.7 Sama koodi saab kirjutada neljal viisil
...
## 1.8 Objektide tüübid 
###1.8.1 Vektor - andmerida
...
Vektoritega saab teha aritmeetilisi tehteid.
```{r}
a <- c(1,2,3)
...
###1.8.3. Tibble ja data frame - andmeraamid
...
## 1.4 Kirjutame oma esimese funktsiooni
...
## 1.5 funktsioonide raamatukogud
...
## 1.11.Regular expression ja find & replace
...
### 1.12.2 Tabeli dimensioonide muutmine (pikk ja lai formaat)
...
#### 1.12.3 Tibble transpose --- read veergudeks ja vastupidi
...
```{r}
iris %>% select(liik=Species, Sepal.Length, Sepal.Width ) %>% head()
```
...
#### 1.13.7. separate() one column into several
