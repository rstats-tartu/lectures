...
Täpsemalt, andmemudel modeleerib, kui tõenäoline oleks kohata just selle valimi keskmist kõikvõimalikel populatsiooni väärtustel. 
See mudel on normaaljaotus, mille keskväärtus võrdub valimiandmete keskväärtusega ja standardhälve võrdub valimi standardhälvega, mis on jagatud ruutjuurega n-st (SEM = sd(valim)/sqrt(n)). 

Taustateadmiste mudel on sageli samuti normaaljaotus. Kui meil on palju taustateadmisi, siis on see jaotus kõrge ja kitsas, kui meil on vähe taustateadmisi, siis on see madal ja lai. Igal juhul järgmise sammuna korrutab bayesiaan selle jaotuse andmejaotusega, saades tulemuseks kolmanda normaaljaotuse, mille ta seejärel normaliseerib nii, et jaotuse alune pindala = 1. 
...
Sageduslik lähenemine seevastu sisaldab ainult ühte mudelit, mida võrreldakse valimi andmetega. 
Sageduslik statistik alustab selles lihtsas näites täpselt samamoodi nagu bayesiaan, tekitades eelmisega identse andmemudeli, mis on keskendatud valimi keskväärtusele. 
Seejärel nihutab ta oma andmemudelit niipalju, et normaaljaotuse tipp ei ole enam valimi keskväärtuse kohal vaid hoopis 0-väärtuse kohal (näiteks 0-efekti kohal). 
Samas jaotuse laius nihutamise käigus ei muutu. Seda nullile tsentreeritud mudelit kutsutakse null-hüpoteesiks (H0). 
Nüüd võrreldakse oma valimi keskväärtust H0 jaotusega. Kui valimi keskväärtuse kohal on H0 jaotus kõrge, siis on andmete tõenäosus H0 kehtimise korral suur. 
Ja vastupidi, kui valimi keskväärtuse kohal on H0 madal, siis on andmete esinemise tõenäosus H0 all madal. 
Seda tõenäosust kutsutakse p väärtuseks. Mida väiksem on p väärtus, seda vähem tõenäolised on teie andmed juhul, kui H0 on tõene ja katseefekt võrdub nulliga. 
...
Paraku ei tea me ühegi üksiku testi kohta ette, kas see testib kehtivat või mittekehtivat H0-i, mis raskeks katseseeriate ühekaupa tõlgendamise. 
...
Kas p väärtusi saab tõlgendada ühekaupa kui hinnangut tõendusmaterjali hulgale, mida teie valim pakub H0 vastu? Selle üle on vaieldud üle 80 aasta, kuid tundub, et ainus viis seda teha, kas või umbkaudu, on bayesiaanlik. Kui te mõtlete p väärtuse definitsioonile, siis peaksite mõistma, miks p väärtust, mis on defineeritud pikaajalise sagedusena, on raske rakendada üksiksündmusele. Bayesiaanliku p väärtuste tõlgendamiskalkulaatori leiate aadressilt ...  

Kujutle mass spektroskoopia katset, kus mõõdame 2000 valgu tasemeid katse-kontroll skeemis ja igat katset korratakse n korda. Sageduslik statistik kasutab adjusteeritud p väärtusi või q väärtusi, et tõmmata piir, millest ühele poole jäävad statistiliselt olulised tulemused (efektid) ja teisele poole statistiliselt mitteolulised null-efektid. Edasi tõlgendab ta vaikimisi mitteolulisi efekte kui ebaolulisi ja diskuteerib vaid "olulisi" efekte (neidki valikuliselt). Paraku, p väärtuste arvutamine ja adjusteerimine saab toimuda mitmel erineval moel ja usalduspiiri panekule 95-le protsendile, mitte näiteks 89% või 99.2%-le, pole ühtegi ratsionaalset põhjendust. Seega järjestab ta oma tulemused efekti suuruse järgi, seejärel tõmbab neist osad selles reas tahapoole (need, mille katsest-katsesse varieeruvus on suurem kui teistel) ja lõpuks tõmbab juhuslikus kohas joone läbi efektide, misjärel ta ignoreerib kõiki sellest juhuslikust joonest valele poole jäänud efekte. Meetod, mis väga hästi töötab pikajalises kvaliteedikontrollis ei ole kahjuks kuigi kasulik katse tulemuste ükshaaval tõlgendamises. Mis juhtub, kui proovime erinevaid p väärtustega töötamise meetodeid, et leida usalduspiir, millest õigele poole jäävaid andmeid on teaduslikult kõige parem tõlgendada? Siis ei fikseeri p väärtused enam tüüp 1 vigade sagedust, ja piiri võiks sama hästi tõmmata tunde järgi, kui erinevaid algoritme proovides.
